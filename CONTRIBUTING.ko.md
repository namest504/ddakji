...
PR 전에 아래가 전부 통과해야 합니다 (pre-push 훅이 같은 것을 봅니다).
...
`README`·`docs/*`·`CONTRIBUTING`은 **영어가 정본**이고 같은 이름의 `.ko.md`가
한국어판입니다. 문서를 고칠 때는 **두 파일을 함께** 고쳐 주세요 — 한쪽만 고친
PR은 나머지 한쪽도 채워 달라고 요청받게 됩니다.
