...
숨기기는 노트를 화면에서만 치웁니다. 숨긴 노트도 파일과 목록에 그대로 있고 —
목록에서는 흐려진 채 **숨김** 칩이 붙어 구별됩니다 — 목록에서 열면 돌아옵니다. 다만 숨겨 둔 동안에는 재시작해도 다시 뜨지 않고, 모음집의 화살표·점에도
걸리지 않습니다.
...
딱지 **뒷면**(오른쪽 아래 빗금 모서리)의 공유 줄에서:

- **서식 복사** — 클립보드에 서식(HTML, 이미지 내장)과 평문 마크다운을 함께 담습니다.
  카톡·메일·노션·워드에는 서식으로, 서식을 못 받는 곳에는 마크다운으로 붙습니다.
- **마크다운** — `.md`로 저장합니다. 이미지가 있으면 `note.md`+`assets/`를 담은
  **zip**이 되어, 받는 쪽에서 풀면 이미지 경로가 바로 통합니다.
- **HTML** — 이미지까지 내장한 **한 파일짜리 HTML**. 더블클릭이면 브라우저에서
  구조·이미지가 그대로 보입니다.

어느 쪽이든 창 위치·폰트 같은 내부 정보(프론트매터)는 담기지 않습니다.

## 업데이트

앱을 켜면 조용히 새 버전을 확인하고, 있으면 **자동으로 내려받아 설치한 뒤
다시 시작합니다** (설치형 전용). 진행 중에는 노트 목록 창 상단에 "설치 중…"이
표시됩니다. 설치에 실패하면 "vX 업데이트" 버튼이 남아 눌러서 다시 시도할 수
있습니다. 포터블로 쓰는 중이면 자동 설치 대신 버튼이 릴리스 페이지를 엽니다.
...
- **정렬(자석)**: 창을 끌어다 놓을 때 다른 노트나 화면 가장자리에 가까우면
  착 붙어 정렬됩니다. 커서가 상대 창 위에 있으면 정렬 대신 합치기입니다.
...
- **언어**: 시스템(OS 언어 추종) / 한국어 / English — 노트·목록·트레이가 함께 바뀝니다
- **새 노트 기본값**: 색·폰트·글씨 크기
- **자주 쓰는 폰트**: "폰트 추가"로 설치된 폰트를 검색해 등록 — 등록한 폰트는 노트 툴바의 Aa 팝오버에 나타납니다
- **AI 연동**: **Claude 스킬 설치**(AI가 노트를 다루는 법을 익힘 — 클릭 한 번),
